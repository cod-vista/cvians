...
function DateFilterPopover({ columnIndex, uniqueValues }: DateFilterPopoverProps) {
  const context = React.useContext(TableContext)
  if (!context) throw new Error('DateFilterPopover must be used within ExcelTable')
  ...
  const [selectedFilters, setSelectedFilters] = React.useState<DateFilter[]>(currentFilters)
    ...
    setDateHierarchy((prevHierarchy) => {
      ...
      prevHierarchy.forEach((year: DateHierarchy) => {
        ...
        year.months.forEach((month: DateHierarchy['months'][0]) => {
          existingState.set(`month-${year.year}-${month.month}`, { selected: month.selected })
          month.days.forEach((day: DateHierarchy['months'][0]['days'][0]) => {
            existingState.set(`day-${day.fullDate}`, { selected: day.selected })
          })
        })
      })
      ...
      uniqueValues.forEach(dateStr => {
        ...
        if (!monthData) {
          const existingMonthState = existingState.get(`month-${year}-${month}`)
          monthData = {
            month,
  ...
  const handleYearToggle = React.useCallback((yearIndex: number, checked?: boolean | "indeterminate") => {
    setDateHierarchy((prev: DateHierarchy[]) => {
      const newHierarchy = [...prev]
      const yearData = newHierarchy[yearIndex]
      yearData.selected = checked === true ? true : checked === false ? false : !yearData.selected
  ...
  const handleMonthToggle = React.useCallback((yearIndex: number, monthIndex: number, checked?: boolean | "indeterminate") => {
    setDateHierarchy((prev: DateHierarchy[]) => {
      const newHierarchy = [...prev]
      const yearData = newHierarchy[yearIndex]
      const monthData = yearData.months[monthIndex]
      monthData.selected = checked === true ? true : checked === false ? false : !monthData.selected
      ...
      yearData.selected = yearData.months.every((m: DateHierarchy['months'][0]) => m.selected)
  ...
  const handleDayToggle = React.useCallback((yearIndex: number, monthIndex: number, dayIndex: number, checked?: boolean | "indeterminate") => {
    setDateHierarchy((prev: DateHierarchy[]) => {
      const newHierarchy = [...prev]
      const yearData = newHierarchy[yearIndex]
      const monthData = yearData.months[monthIndex]
      const dayData = monthData.days[dayIndex]
      dayData.selected = checked === true ? true : checked === false ? false : !dayData.selected

      // Update month selection based on days
      monthData.selected = monthData.days.every((d: DateHierarchy['months'][0]['days'][0]) => d.selected)
    ...
    setDateHierarchy((prev: DateHierarchy[]) => {
      return prev.map((year: DateHierarchy) => ({
        ...
        months: year.months.map((month: DateHierarchy['months'][0]) => ({
          ...month,
          selected: false,
          days: month.days.map((day: DateHierarchy['months'][0]['days'][0]) => ({
            ...day,
            selected: false
          }))
        }))
...
export function ExcelTable({ children, className }: ExcelTableProps) {
  ...
  const getFilteredAndSortedData = React.useCallback((): ReactNode[] => {
    ...
    const sortEntries = Object.entries(sorts).filter(([, direction]) => direction !== null && direction !== undefined)
                    ...
                    <div className="space-y-2 max-h-64 overflow-y-auto">
                      {uniqueValues.map(value => (
                        <div key={value} className="flex items-center space-x-2">
                          ...
                          >
                            {dataType === 'boolean' 
                              ? value === 'true' || value === 'yes' || value === 'Yes' ? '✓ True' : value === 'false' || value === 'no' || value === 'No' ? '✗ False' : value
                              : value || '(Empty)'
                            }
                          </label>
                        </div>
